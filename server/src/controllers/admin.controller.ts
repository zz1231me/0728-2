// src/controllers/admin.controller.ts - 최종 보안 강화 버전 (타입 에러 해결)
...
import { sequelize } from '../config/sequelize'; // 🔧 추가된 import
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    // 🔒 삭제된 사용자 계정 삭제 방지
    if (id === 'deleted_user') {
      return res.status(400).json({ message: '[삭제된 사용자] 계정은 삭제할 수 없습니다.' });
    }
    ...
    try {
      // 1. [삭제된 사용자] 계정 확인/생성
      const { createDeletedUserAccount } = await import('../scripts/migrate-data');
      await createDeletedUserAccount();

      // 2. 해당 사용자의 게시글을 [삭제된 사용자]로 이관
      ...
      try {
        ...
        const updatedPosts = await Post.update(
          { 
            UserId: 'deleted_user',
            author: '[삭제된 사용자]' // author 필드도 함께 업데이트
          },
          { 
            where: { UserId: id },
            transaction 
        ...
        updatedPostsCount = updatedPosts[0];
      } catch (error) {
        console.warn('⚠️ 게시글 이관 중 에러:', error);
      }

      // 3. 해당 사용자의 댓글을 [삭제된 사용자]로 이관
      ...
      try {
        ...
        updatedCommentsCount = updatedComments[0];
      } catch (error) {
        console.warn('⚠️ 댓글 이관 중 에러:', error);
      }

      // 4. 해당 사용자의 이벤트를 [삭제된 사용자]로 이관 (선택사항)
      ...
      try {
        ...
        updatedEventsCount = updatedEvents[0];
      } catch (eventError) {
        console.warn('⚠️ 이벤트 이관 중 에러 (계속 진행):', eventError);
      }

      // 5. 이제 사용자 삭제 (CASCADE 관계가 없으므로 안전)
      ...
      logSecurityEvent('USER_DELETED_WITH_CONTENT_PRESERVATION', requestUser.id, { 
        deletedUserId: id, 
        deletedUserName: targetUser.name,
        deletedUserRole: targetUser.roleId,
        preservedPosts: updatedPostsCount,
        preservedComments: updatedCommentsCount,
        preservedEvents: updatedEventsCount
      });

      res.json({ 
        message: '사용자가 삭제되었습니다. 작성한 게시글과 댓글은 "[삭제된 사용자]"로 표시됩니다.',
        details: {
...
export const deleteBoard = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      // 2. BoardAccess 삭제
      const deletedAccessCount = await BoardAccess.destroy({ 
        where: { boardId: id },
        transaction 
      });

      // 3. 게시판 삭제
      const deletedBoardCount = await Board.destroy({ 
        where: { id },
        transaction 
      });
