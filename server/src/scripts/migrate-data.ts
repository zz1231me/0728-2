...
// ✅ [삭제된 사용자] 계정 생성 함수 추가
export const createDeletedUserAccount = async () => {
  try {
    // 이미 존재하는지 확인
    const existingDeletedUser = await User.findByPk('deleted_user');
    ...
    let guestRole = await Role.findByPk('guest');
    if (!guestRole) {
      // guest 역할이 없으면 group1 역할 사용
      guestRole = await Role.findByPk('group1');
      if (!guestRole) {
        console.warn('⚠️ guest나 group1 역할을 찾을 수 없습니다. 첫 번째 역할을 사용합니다.');
        guestRole = await Role.findOne();
      }
    }
    ...
    await User.create({
      id: 'deleted_user',
      password: hashPassword('deleted_account_' + Date.now()), // 로그인 불가능한 비밀번호
      name: '[삭제된 사용자]',
