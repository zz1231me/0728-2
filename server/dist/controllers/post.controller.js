...
const getPosts = async (req, res) => {
    try {
        ...
        const whereCondition = { boardType };
        if (search) {
        ...
        const totalCount = await Post_1.Post.count({ where: whereCondition });
...
const getPostById = async (req, res) => {
    try {
        const post = await Post_1.Post.findByPk(req.params.id, {
            include: [{
                    model: User_1.User,
                    as: 'user',
                    attributes: ['id', 'name']
...
const createPost = async (req, res) => {
    try {
        ...
        const cleanContent = (0, sanitize_html_1.default)(content, sanitizeOptions);
        // ✅ 한글 파일명 완벽 처리
        ...
        const postWithUser = await Post_1.Post.findByPk(post.id, {
            include: [{
                    model: User_1.User,
                    as: 'user',
                    attributes: ['id', 'name']
...
const updatePost = async (req, res) => {
    try {
        ...
        if (parsedDeletedFileNames.length > 0) {
            ...
            existingFiles = existingFiles.filter(file => !parsedDeletedFileNames.includes(file.storedName));
        ...
        const updatedPost = await Post_1.Post.findByPk(id, {
            include: [{
                    model: User_1.User,
                    as: 'user',
                    attributes: ['id', 'name']
        ...
        if (post.attachments) {
            try {
                ...
                if (Array.isArray(files)) {
                    files.forEach((file) => deleteFileIfExists(file.storedName));
                }
            }
            catch (error) {
