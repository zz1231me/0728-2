...
const deleteUser = async (req, res) => {
    try {
        ...
        try {
            // 1. [삭제된 사용자] 계정 확인/생성
            const { createDeletedUserAccount } = await Promise.resolve().then(() => __importStar(require('../scripts/migrate-data')));
            ...
            try {
                const updatedEvents = await Event_1.default.update({
                    UserId: 'deleted_user'
                }, {
                    where: { UserId: id },
                    transaction
            ...
            const deleted = await User_1.User.destroy({
                where: { id },
                transaction
            });
            if (deleted === 0) {
