...
// 🔧 토큰 만료 시간 상수 정의 (통일)
const ACCESS_TOKEN_MINUTES = 15; // Access Token: 15분
const REFRESH_TOKEN_DAYS = 3; // Refresh Token: 3일
...
const login = async (req, res, next) => {
    try {
        ...
        const accessToken = jsonwebtoken_1.default.sign(payload, process.env.JWT_SECRET, { expiresIn: `${ACCESS_TOKEN_MINUTES}m`, algorithm: 'HS256' });
        // 🔄 Refresh Token (3일) - 별도 시크릿 사용
        const refreshToken = jsonwebtoken_1.default.sign({ id: user.id, tokenType: 'refresh' }, process.env.JWT_REFRESH_SECRET, { expiresIn: `${REFRESH_TOKEN_DAYS}d`, algorithm: 'HS256' });
        // 🍪 HttpOnly 쿠키로 토큰 저장
        res.cookie('access_token', accessToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'strict',
            maxAge: ACCESS_TOKEN_MINUTES * 60 * 1000 // 15분
        });
        res.cookie('refresh_token', refreshToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'strict',
            maxAge: REFRESH_TOKEN_DAYS * 24 * 60 * 60 * 1000 // 3일
        });
        ...
        res.json({
            ...
            // ✅ 토큰 정보 - 실제 만료 시간과 일치하도록 수정
            tokenInfo: {
                accessTokenExpiry: new Date(Date.now() + ACCESS_TOKEN_MINUTES * 60 * 1000).getTime(), // ✅ 15분으로 수정
                refreshTokenExpiry: new Date(Date.now() + REFRESH_TOKEN_DAYS * 24 * 60 * 60 * 1000).getTime() // 3일
            }
        });
...
const refreshToken = async (req, res) => {
    try {
        ...
        const payload = await (0, admin_controller_1.generateUserPayload)(userForPayload);
        // ✅ 새 Access Token - 15분으로 통일
        const newAccessToken = jsonwebtoken_1.default.sign(payload, process.env.JWT_SECRET, { expiresIn: `${ACCESS_TOKEN_MINUTES}m`, algorithm: 'HS256' });
        // 새 Access Token을 쿠키에 저장
        res.cookie('access_token', newAccessToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            sameSite: 'strict',
            maxAge: ACCESS_TOKEN_MINUTES * 60 * 1000 // ✅ 15분으로 수정
        });
        res.json({
            message: '토큰 갱신 성공',
            user: {
