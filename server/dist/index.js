...
const setupModelAssociations = () => {
    ...
    // 2️⃣ User ↔ Post 관계 (🔧 CASCADE → RESTRICT 변경)
    User_1.User.hasMany(Post_1.Post, {
        foreignKey: 'UserId',
        as: 'posts',
        onDelete: 'RESTRICT' // ✅ CASCADE → RESTRICT로 변경
    });
    ...
    // 3️⃣ Post ↔ Comment 관계 (변경 없음 - 게시글 삭제 시 댓글도 삭제되어야 함)
    ...
    // 4️⃣ User ↔ Comment 관계 (🔧 CASCADE → RESTRICT 변경)
    ...
    // 8️⃣ User ↔ Event 관계 (🔧 CASCADE → RESTRICT 변경)
    ...
    console.log('✅ 모든 모델 관계 설정 완료 (사용자 삭제 시 컨텐츠 보존)');
};
// ✅ 미들웨어 (순서 중요!)
