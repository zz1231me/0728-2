...
exports.runMigrationIfNeeded = exports.migrateData = exports.createDeletedUserAccount = void 0;
...
// ✅ [삭제된 사용자] 계정 생성 함수 추가
const createDeletedUserAccount = async () => {
    try {
        // 이미 존재하는지 확인
        const existingDeletedUser = await User_1.User.findByPk('deleted_user');
        if (existingDeletedUser) {
            console.log('✅ [삭제된 사용자] 계정이 이미 존재합니다.');
            return;
        }
        // guest 역할이 있는지 확인 (없으면 기본 역할 사용)
        let guestRole = await Role_1.Role.findByPk('guest');
        if (!guestRole) {
            // guest 역할이 없으면 group1 역할 사용
            guestRole = await Role_1.Role.findByPk('group1');
            if (!guestRole) {
                console.warn('⚠️ guest나 group1 역할을 찾을 수 없습니다. 첫 번째 역할을 사용합니다.');
                guestRole = await Role_1.Role.findOne();
            }
        }
        // [삭제된 사용자] 계정 생성
        await User_1.User.create({
            id: 'deleted_user',
            password: (0, hash_1.hashPassword)('deleted_account_' + Date.now()), // 로그인 불가능한 비밀번호
            name: '[삭제된 사용자]',
            roleId: guestRole?.id || 'group1'
        });
        console.log('✅ [삭제된 사용자] 계정이 생성되었습니다.');
    }
    catch (error) {
        console.error('❌ [삭제된 사용자] 계정 생성 실패:', error);
    }
};
exports.createDeletedUserAccount = createDeletedUserAccount;
const migrateData = async () => {
    ...
    try {
        ...
        if (existingRoles > 0 && existingBoards > 0 && existingEventPermissions > 0) {
            ...
            // ✅ 기존 데이터가 있어도 [삭제된 사용자] 계정은 확인
            await (0, exports.createDeletedUserAccount)();
        ...
        // ✅ 3. [삭제된 사용자] 계정 생성
        console.log('👤 [삭제된 사용자] 계정 생성 중...');
        await (0, exports.createDeletedUserAccount)();
        // 4. 기본 게시판(Board) 데이터 생성 - 🔧 admin 게시판 추가
        ...
        console.log('');
        console.log('👤 특수 계정:');
        console.log('   🗑️ [삭제된 사용자]: 사용자 삭제 시 게시글/댓글 보존용');
        console.log('');
    }
    catch (error) {
